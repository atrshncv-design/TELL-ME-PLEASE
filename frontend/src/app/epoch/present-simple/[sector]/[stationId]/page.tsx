...
async function loadSector(sectorId: string): Promise<EpochSector | null> {
...
function StationStub({ sector }: { sector: string }) {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-gradient-to-br from-violet-100 via-sky-50 to-amber-50 px-4">
      <a
        href={`/epoch/present-simple/${sector}`}
...
export default async function EpochStationPage({
  ...
  const sectorMeta = await loadSector(sector)
  ...
  const backHref = `/epoch/present-simple/${sector}`

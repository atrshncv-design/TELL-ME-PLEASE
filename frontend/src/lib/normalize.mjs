/**
 * Normalize raw task JSON to match component contracts.
 *
 * CANONICAL SOURCE OF TRUTH.
 *
 * This plain-ESM module (no React/Next deps, importable from both the Next.js
 * runtime and plain Node) is the single implementation of task normalization.
 * Two consumers both reach this file so the harness validates exactly what the
 * live app renders:
 *
 *   - frontend/src/lib/normalize-task.ts -> typed re-export of normalize (here)
 *   - scripts/normalize.mjs             -> re-export of normalize (here), used
 *                                          by scripts/verify-content.mjs
 *
 * Do NOT duplicate this logic. If a task type needs new normalization, edit
 * THIS file only.
 *
 * Note on location: the canonical file lives inside the frontend tree because
 * Turbopack confines module resolution to the frontend project root and cannot
 * bundle a module reached via paths like `../../../scripts/...`. Node scripts

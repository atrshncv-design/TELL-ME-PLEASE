...
export interface TaskData {
  ...
  sentenceBuilder?: SentenceBuilderData;
...
/** Роль/тип столбика-банка sentence-builder (T05): какой член предложения
 *  собирает этот столбик. 1 тип = 1 столбик. */
export type SentenceBuilderRole = "subject" | "verb" | "object" | "adverbial";

/** Один столбик-банк sentence-builder (T05): слова определённого типа. */
export interface SentenceBuilderColumn {
  /** Роль столбика: subject (Подлежащее) / verb (Сказуемое) /
   *  object (Дополнение) / adverbial (Обстоятельство). */
  role: SentenceBuilderRole;
  /** Подпись по-русски («Подлежащее»). */
  label: string;
  /** Подсказка-вопрос («кто/что?»). */
  hint?: string;
  /** Слова-чипы этого столбика — в форме как в предложении
   *  (пустой массив = столбик пропускается в UI). */
  words: string[];
}

/** Одно предложение sentence-builder (T05): 4 столбика-банка + правильный
 *  ответ. Ребёнок выбирает по одному слову из каждого непустого столбика —
 *  предложение собирается в строке-результате ВЫШЕ столбиков. */
export interface SentenceBuilderItem {
  columns: SentenceBuilderColumn[];
  /** Правильное предложение (дословно из документа, с точкой). */
  answer: string;
}

/** sentence-builder (T05, станция A1.5 «Визуализация Синтаксиса»): построй
 *  предложение, выбрав слова из 4 столбиков. Счёт = верно собранные
 *  предложения / items.length. */
export interface SentenceBuilderData {
  items: SentenceBuilderItem[];
}

/** Один дополнительный блок карточки-промпта (T06): заголовок + строки
 *  («Примерные вопросы для разговора», «Страны и праздники»). */
export interface PromptCardBlock {
  /** Заголовок блока по-русски («Примерные вопросы для разговора»). */
  title: string;
  /** Строки блока дословно из документа (вопросы / страны и праздники). */
  lines: string[];
}

/** Карточка-промпт (T06, станции A1/A2/B1/B2 эпохи Present Simple): «Промпт
 *  для внешнего чата» — текст дословно + кнопка «Скопировать» + кнопка
 *  «Я поговорил — отметить выполненным» (зачёт по факту, без ИИ-проверки). */
export interface PromptCardData {
  /** Текст промпта дословно из документа (для вставки во внешний чат). */
  prompt: string;
  /** Дополнительные блоки под промптом (опционально, B1/B2). */

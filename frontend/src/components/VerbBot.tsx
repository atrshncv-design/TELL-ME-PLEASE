...
const PHRASES: Record<BotEventType, string[]> = {
  greet: ["Hi! I'm Verb Bot! 🤖", "Welcome! Let's learn verbs!"],
  start: ["Let's go! 🚀", "Scan the verb! 🔍", "Ready? Let's do it!"],
  correct: ["Great job! ⭐", "Verb power activated! ⚡", "Perfect! 🎉", "You got it! 👍"],
...
export function VerbBotProvider({ children }: { children: React.ReactNode }) {
  ...
  const say = useCallback((event: BotEventType) => {
      ...
      <AnimatePresence>
        {phrase && (
          <motion.div
            ...
            className="max-w-[200px] bg-white rounded-2xl rounded-br-md px-3 py-2 text-sm font-medium text-slate-700 shadow-lg border border-indigo-100"
          >
            {phrase}
          </motion.div>
        )}
